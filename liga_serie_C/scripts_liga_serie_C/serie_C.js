...
(function () {
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    function getRodadaEmAndamento() {
      const metaRodada = window.ligaSerieCMeta?.rodada_atual ?? window.rodada_atual ?? window.rodadaAtual;
      if (Number.isFinite(metaRodada)) return metaRodada;

      const rodadaParcialMeta = window.ligaSerieCMeta?.rodada_parcial;
    ...
    function isParcial(rodadaReal) {
      const rodadaParcial = window.ligaSerieCMeta?.rodada_parcial;
      ...
      if (window.ligaSerieCMeta?.parcial_disponivel === true) {
    ...
    function getParticipantesSerieA() {
      ...
      const fontes = [
        pick("participantesLigaSerieC"),
        pick("participantesLiga_serie_C"),
        pick("participantesLigaSerie_C"),
    ...
    function getClassificacaoFonte() {
      const fonte = (typeof classificacaoSerieC === "object" && classificacaoSerieC) ? classificacaoSerieC : null;
    ...
    function renderPainelCompleto(numeroRodada) {
      ...
      const usarAvisoParcial = Number.isFinite(rodadaParcial)
        ? (numeroRodada === rodadaParcial && numeroRodada === rodadaEmAndamento)
