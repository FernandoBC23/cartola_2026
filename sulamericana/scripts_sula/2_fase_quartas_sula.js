window.addEventListener("DOMContentLoaded", () => {
  ...
  const rodadasDisponiveis = Array.from(
    new Set([
      ...(Array.isArray(confrontos_quartas_sula) ? confrontos_quartas_sula.map((j) => Number(j?.rodada)) : []),
      ...(Array.isArray(resultados_quartas_sula) ? resultados_quartas_sula.map((j) => Number(j?.rodada)) : []),
    ].filter(Number.isFinite))
  ).sort((a, b) => a - b);
  const RODADA_MINIMA = rodadasDisponiveis[0] ?? 34;
  const RODADA_MAXIMA = rodadasDisponiveis[rodadasDisponiveis.length - 1] ?? RODADA_MINIMA;
  ...
  const rodadasEncerradas = resultados_quartas_sula
    .filter((r) => {
