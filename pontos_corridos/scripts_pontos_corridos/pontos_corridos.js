...
(function () {
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    function getRodadaEmAndamento() {
      const metaRodada = window.pontosCorridosMeta?.rodada_atual ?? window.rodada_atual ?? window.rodadaAtual;
      if (Number.isFinite(metaRodada)) return metaRodada;

      const rodadaParcialMeta = window.pontosCorridosMeta?.rodada_parcial;
    ...
    function isParcial(rodadaReal) {
      ...
      if (window.pontosCorridosMeta?.parcial_disponivel === true) {
    ...
    function renderBannerParcial(container, rodadaReal) {
      ...
      aviso.textContent = `Rodada ${rodadaReal} em andamento: pontuacoes parciais (nao definitivas).`;
    ...
    function renderPainelCompleto(numeroRodada) {
      ...
      // === resumo da rodada (no padr?o da Liga Elimina??o) ===
